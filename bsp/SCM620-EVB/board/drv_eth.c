...
static struct rt_scm630_eth scm630_eth_device;
...
static rt_err_t rt_scm630_eth_init(rt_device_t dev)
{
...
static rt_err_t rt_scm630_eth_open(rt_device_t dev, rt_uint16_t oflag)
{
    MAC_TRACE("emac open\n");
    return RT_EOK;
}

static rt_err_t rt_scm630_eth_close(rt_device_t dev)
{
    MAC_TRACE("emac close\n");
    return RT_EOK;
}

static rt_size_t rt_scm630_eth_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    MAC_TRACE("emac read\n");
...
static rt_size_t rt_scm630_eth_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
...
static rt_err_t rt_scm630_eth_control(rt_device_t dev, int cmd, void *args)
{
    switch (cmd)
    {
    case NIOCTL_GADDR:
        /* get mac address */
        if (args) rt_memcpy(args, scm630_eth_device.dev_addr, 6);
...
/* transmit data*/
rt_err_t rt_scm630_eth_tx(rt_device_t dev, struct pbuf *p)
{
...
/* receive data*/
struct pbuf *rt_scm630_eth_rx(rt_device_t dev)
{
		struct pbuf *p = NULL;
...
//	{
//		MAC_TRACE("RX\n");
		eth_device_ready(&(scm630_eth_device.parent));
//	}
  ETH_DMAClearITPendingBit(SGCC_GMAC1_P, ETH_DMA_IT_R);     //清除DMA中断标志位
  ETH_DMAClearITPendingBit(SGCC_GMAC1_P, ETH_DMA_IT_NIS);   //清除DMA接收中断标志位
	rt_interrupt_leave();
}
...
static void phy_monitor_thread_entry(void *parameter)
{
    ...
    MAC_TRACE("RESET PHY!\n");
		...
		if(ETH == SGCC_GMAC0_P)
		{
				(*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + ETH_MAC_Address0)) = ((uint32_t)scm630_eth_device.dev_addr[5] << 8) | (uint32_t)scm630_eth_device.dev_addr[4];
		}
		else
		{
				(*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + ETH_MAC_Address0)) = ((uint32_t)scm630_eth_device.dev_addr[5] << 8) | (uint32_t)scm630_eth_device.dev_addr[4];
		}   
		/* Load the selectecd MAC address low register */
		if(ETH == SGCC_GMAC0_P)
		{
				(*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x44 + ETH_MAC_Address0)) = ((uint32_t)scm630_eth_device.dev_addr[3] << 24) | ((uint32_t)scm630_eth_device.dev_addr[2] << 16) | ((uint32_t)scm630_eth_device.dev_addr[1] << 8) | scm630_eth_device.dev_addr[0];
		}
		else
		{
				(*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x44 + ETH_MAC_Address0)) = ((uint32_t)scm630_eth_device.dev_addr[3] << 24) | ((uint32_t)scm630_eth_device.dev_addr[2] << 16) | ((uint32_t)scm630_eth_device.dev_addr[1] << 8) | scm630_eth_device.dev_addr[0];
		}  
...
//            MAC_TRACE("PHY Control/Status REG:0x%04X\n", SR);
        ...
        if (phy_speed_new != phy_speed)
        {
            if (phy_speed_new & PHY_LINK_MASK)
            {
                ...
                {
                    MAC_TRACE("100Mbps\n");
                    scm630_eth_device.ETH_Speed = ETH_Speed_100M;
                }
                else
                {
                    scm630_eth_device.ETH_Speed = ETH_Speed_10M;
                    MAC_TRACE("10Mbps\n");
                }
								...
								if (phy_speed_new & PHY_FULL_DUPLEX_MASK)
                {
                    ...
                    scm630_eth_device.ETH_Mode = ETH_MODE_FULLDUPLEX;
                }
                else
                {
                    MAC_TRACE("half-duplex\n");
                    scm630_eth_device.ETH_Mode = ETH_MODE_HALFDUPLEX;
                }
								
								
                /* send link up. */
                eth_device_linkchange(&scm630_eth_device.parent, RT_TRUE);
            } /* link up. */
            else
            {
                MAC_TRACE("link down\n");
                /* send link down. */
                eth_device_linkchange(&scm630_eth_device.parent, RT_FALSE);
            }
...
#define RT_USING_MAC1
/* Register the EMAC device */
static int rt_hw_scm630_eth_init(void)
{
    ...
    scm630_eth_device.dev_addr[0] = 0x00;
    scm630_eth_device.dev_addr[1] = 0x80;
    scm630_eth_device.dev_addr[2] = 0xE1;
    /* generate MAC addr from 96bit unique ID (only for test). */
#ifdef RT_USING_MAC0		
		ETH = SGCC_GMAC0_P;
    scm630_eth_device.dev_addr[3] = 0x44 + ETH_MAC_Address0;
    scm630_eth_device.dev_addr[4] = 0x55 + ETH_MAC_Address0;
    scm630_eth_device.dev_addr[5] = 0x66 + ETH_MAC_Address0;
#endif
#ifdef RT_USING_MAC1		
		ETH = SGCC_GMAC1_P;
    scm630_eth_device.dev_addr[3] = 0x44 + ETH_MAC_Address0;
    ...
    scm630_eth_device.parent.parent.init       = rt_scm630_eth_init;
    scm630_eth_device.parent.parent.open       = rt_scm630_eth_open;
    scm630_eth_device.parent.parent.close      = rt_scm630_eth_close;
    scm630_eth_device.parent.parent.read       = rt_scm630_eth_read;
    scm630_eth_device.parent.parent.write      = rt_scm630_eth_write;
    scm630_eth_device.parent.parent.control    = rt_scm630_eth_control;
    scm630_eth_device.parent.parent.user_data  = RT_NULL;

    scm630_eth_device.parent.eth_rx     = rt_scm630_eth_rx;
    scm630_eth_device.parent.eth_tx     = rt_scm630_eth_tx;

    /* register eth device */
    state = eth_device_init(&(scm630_eth_device.parent), "eth0");
    if (RT_EOK == state)
    {
...
INIT_DEVICE_EXPORT(rt_hw_scm630_eth_init);
...
void tcpclient(int argc, char **argv)
{
    ...
    const char *url;
    int port;
    const char send_data[] = "This is scm630 TCP Test!\n";
    /* 接收到的参数小于 3 个 */
    if (argc < 3)
    {
