...
/**
  ...
  */
uint32_t ETH_HandleTxPkt(ETH_TypeDef *ETH, uint8_t *ppkt, uint16_t FrameLength)
{ 
  uint32_t offset = 0;
  ...
  {  
    if((DMATxDescToSet->ControlBufferSize & ETH_DMATxDesc_TER) != (uint32_t)RESET)
    {
...
/**
  ...
  */
uint32_t ETH_HandleRxPkt(ETH_TypeDef *ETH, uint8_t *ppkt)
{ 
...
/**
  ...
  */
uint32_t ETH_PHYLoopBackCmd(ETH_TypeDef *ETH, uint16_t PHYAddress, FunctionalState NewState)
{
  uint16_t tmpreg = 0;
  /* Check the parameters */
  assert_param(IS_ETH_PHY_ADDRESS(PHYAddress));
  ...
  tmpreg = ETH_ReadPHYRegister(ETH, PHYAddress, PHY_BCR); 
  ...
  if(ETH_WritePHYRegister(ETH, PHYAddress, PHY_BCR, tmpreg) != (uint32_t)RESET)
  {
    return ETH_SUCCESS;
  }
  else
...
/**
  ...
  * @retval None
  */
void ETH_MACAddressConfig(ETH_TypeDef *ETH, uint32_t MacAddr, uint8_t *Addr)
{
  uint32_t tmpreg;
  /* Check the parameters */
  ...
  if(ETH == SGCC_GMAC0_P)
  {
      (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) = tmpreg;
  }
  else
  {
      (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) = tmpreg;
  }   
  ...
  if(ETH == SGCC_GMAC0_P)
  {
      (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x44 + MacAddr)) = tmpreg;
  }
  else
  {
      (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x44 + MacAddr)) = tmpreg;
  }   
}
...
/**
  ...
  * @retval None
  */
void ETH_GetMACAddress(ETH_TypeDef *ETH, uint32_t MacAddr, uint8_t *Addr)
{
  uint32_t tmpreg;
  /* Check the parameters */
  assert_param(IS_ETH_MAC_ADDRESS0123(MacAddr));
  ...
  if(ETH == SGCC_GMAC0_P)
  {
      tmpreg = (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) ;
  }
  else
  {
      tmpreg = (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr));
  }   
  ...
  if(ETH == SGCC_GMAC0_P)
  {
      tmpreg = (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x44 + MacAddr)) ;
  }
  else
  {
      tmpreg = (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x44 + MacAddr));
  }   
  /* Calculate the selectecd MAC address buffer */
...
void ETH_MACAddressPerfectFilterCmd(ETH_TypeDef *ETH, uint32_t MacAddr, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* Enable the selected ETHERNET MAC address for perfect filtering */
     if(ETH == SGCC_GMAC0_P)
     {
       (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) |= ETH_MACA1HR_AE;
     }
     else
     {
       (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) |= ETH_MACA1HR_AE;
     }   
  }
  else
  {
    /* Disable the selected ETHERNET MAC address for perfect filtering */
      if(ETH == SGCC_GMAC0_P)
      {
        (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_AE);
      }
      else
      {
        (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_AE);
      }   
  }
}
...
void ETH_MACAddressFilterConfig(ETH_TypeDef *ETH, uint32_t MacAddr, uint32_t Filter)
{
  ...
  if (Filter != ETH_MAC_AddressFilter_DA)
  {
    /* The selected ETHERNET MAC address is used to compare with the SA fields of the
     ...
     if(ETH == SGCC_GMAC0_P)
     {
        (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) |= ETH_MACA1HR_SA;
     }
     else
     {
        (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) |= ETH_MACA1HR_SA;
     }    
  ...
  else
  {
    /* The selected ETHERNET MAC address is used to compare with the DA fields of the
     ...
     if(ETH == SGCC_GMAC0_P)
     {
        (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_SA);
     }
     else
     {
        (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_SA);
     }    
  }
}
...
void ETH_MACAddressMaskBytesFilterConfig(ETH_TypeDef *ETH, uint32_t MacAddr, uint32_t MaskByte)
{
  ...
  if(ETH == SGCC_GMAC0_P)
  {
     ...
     (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_MBC);
      /* Set the selected Filetr mask bytes */
     (*(__IO uint32_t *) (SGCC_GMAC0_BASE + 0x40 + MacAddr)) |= MaskByte;
  }
  else
  {
     (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) &=(~(uint32_t)ETH_MACA1HR_MBC);
     (*(__IO uint32_t *) (SGCC_GMAC1_BASE + 0x40 + MacAddr)) |= MaskByte;
  }    
...
/**
  ...
  * @retval None
  */
void ETH_DMATxDescRingInit(ETH_TypeDef *ETH, ETH_DMADESCTypeDef *DMATxDescTab, uint8_t *TxBuff1, uint8_t *TxBuff2, uint32_t TxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADESCTypeDef *DMATxDesc;
  ...
  for(i=0; i < TxBuffCount; i++)
  {
    ...
    if(i == (TxBuffCount-1))
    {
      ...
      DMATxDesc->ControlBufferSize |= ETH_DMATxDesc_TER;
    }
  }
...
void ETH_DMATxDescTransmitITConfig(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    DMATxDesc->ControlBufferSize |= ETH_DMATxDesc_IC;
  }
  else
  {
    /* Disable the DMA Tx Desc Transmit interrupt */
    DMATxDesc->ControlBufferSize &=(~(uint32_t)ETH_DMATxDesc_IC);
  }
}
...
void ETH_DMATxDescFrameSegmentConfig(ETH_DMADESCTypeDef *DMATxDesc, uint32_t DMATxDesc_FrameSegment)
{
  ...
  DMATxDesc->ControlBufferSize |= DMATxDesc_FrameSegment;
}
...
void ETH_DMATxDescCRCCmd(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    DMATxDesc->ControlBufferSize &= (~(uint32_t)ETH_DMATxDesc_DC);
  }
  else
  {
    /* Disable the selected DMA Tx Desc CRC */
    DMATxDesc->ControlBufferSize |= ETH_DMATxDesc_DC; 
  }
}
...
void ETH_DMATxDescEndOfRingCmd(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  ...
  else
  {
    ...
    DMATxDesc->ControlBufferSize &= (~(uint32_t)ETH_DMATxDesc_TER); 
  }
}
...
void ETH_DMATxDescSecondAddressChainedCmd(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  ...
  else
  {
    ...
    DMATxDesc->ControlBufferSize &=(~(uint32_t)ETH_DMATxDesc_TCH); 
  }
}
...
void ETH_DMATxDescShortFramePaddingCmd(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    DMATxDesc->ControlBufferSize &= (~(uint32_t)ETH_DMATxDesc_DP);
  }
  else
  {
    /* Disable the selected DMA Tx Desc padding for frame shorter than 64 bytes*/
    DMATxDesc->ControlBufferSize |= ETH_DMATxDesc_DP; 
  }
}
...
void ETH_DMATxDescTimeStampCmd(ETH_DMADESCTypeDef *DMATxDesc, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    DMATxDesc->ControlBufferSize |= ETH_DMATxDesc_TTSE;  
  }
  else
  {
    /* Disable the selected DMA Tx Desc time stamp */
    DMATxDesc->ControlBufferSize &=(~(uint32_t)ETH_DMATxDesc_TTSE); 
  }
}
...
/**
  ...
  * @retval None
  */
void ETH_DMARxDescRingInit(ETH_TypeDef *ETH, ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff1, uint8_t *RxBuff2, uint32_t RxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADESCTypeDef *DMARxDesc;
  /* Set the DMARxDescToGet pointer with the first one of the DMARxDescTab list */
...
uint32_t ETH_GetMMCRegister(ETH_TypeDef *ETH, uint32_t ETH_MMCReg)
{
  ...
  if(ETH == SGCC_GMAC0_P)
  {
      return (*(__IO uint32_t *)(SGCC_GMAC0_BASE + ETH_MMCReg));
  }
  else 
  {
      return (*(__IO uint32_t *)(SGCC_GMAC1_BASE + ETH_MMCReg));
  }
...
uint32_t ETH_GetPTPRegister(ETH_TypeDef *ETH, uint32_t ETH_PTPReg)
{
  ...
  if(ETH == SGCC_GMAC0_P)
  {
      return (*(__IO uint32_t *)(SGCC_GMAC0_BASE + ETH_PTPReg));
  }
  else
  {
      return (*(__IO uint32_t *)(SGCC_GMAC1_BASE + ETH_PTPReg));
  }
}
...
/**
  ...
  * @retval None
  */
void ETH_DMAPTPTxDescChainInit(ETH_TypeDef *ETH, ETH_DMADESCTypeDef *DMATxDescTab, ETH_DMADESCTypeDef *DMAPTPTxDescTab,
                               uint8_t* TxBuff, uint32_t TxBuffCount)
{
  ...
  for(i=0; i < TxBuffCount; i++)
  {
    ...
    DMATxDesc->ControlBufferSize = ETH_DMATxDesc_TCH | ETH_DMATxDesc_TTSE;  
...
/**
  ...
  * @retval None
  */
void ETH_DMAPTPRxDescChainInit(ETH_TypeDef *ETH, ETH_DMADESCTypeDef *DMARxDescTab, ETH_DMADESCTypeDef *DMAPTPRxDescTab,
...
/**
  ...
  *         ETH_SUCCESS: for correct transmission
  */
uint32_t ETH_HandlePTPTxPkt(ETH_TypeDef *ETH, uint8_t *ppkt, uint16_t FrameLength, uint32_t *PTPTxTab)
{
  ...
  DMATxDescToSet->ControlBufferSize |= (FrameLength & (uint32_t)0x7FF);
  /* Setting the last segment and first segment bits (in this case a frame is transmitted in one descriptor) */    
  DMATxDescToSet->ControlBufferSize |= ETH_DMATxDesc_LS | ETH_DMATxDesc_FS;
  /* Set Own bit of the Tx descriptor Status: gives the buffer back to ETHERNET DMA */
  DMATxDescToSet->Status |= ETH_DMATxDesc_OWN;
...
/**
  ...
  *         framelength: received packet size if packet reception is correct
  */
uint32_t ETH_HandlePTPRxPkt(ETH_TypeDef *ETH, uint8_t *ppkt, uint32_t *PTPRxTab)
{
  uint32_t offset = 0, framelength = 0;
